        ...
        connection.on("Broadcast",
            function (response) {
                $("#chatRoom").append(
                    ...
                    '<p class="small text-muted">' + response.messageDatetime + '</p>' +
                    '</div>' +
                    '</div>'
                );
        ...
        connection.on("UserConnected",
            function (response) {
                ...
                $("#chatRoom").append(
                    ...
                    '<p class="text-small mb-0 text-muted">' + response.userName + ' - Welcome to the Chat Room!</p>' +
                    '</div>' +
                    '<p class="small text-muted">' + response.messageDatetime + '</p>' +
                    '</div>'
                );
                ...
                $("#chatUsers").append(
                    ...
                    '<h6 class="mb-0">' + response.userName + '</h6><small class="small font-weight-bold">' +
                    response.connectionDatetime +
                    '</small>' +
                    '</div>' +
